...
const RenewallogsPage = (props) => {
    ...
    useEffect(() => {
        ...
        client
            .service("renewallogs")
            .find({ query: { $limit: 10000 , userID : urlParams.singleMembersId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            ...
            console.log({ error });
            props.alert({ title: "Renewallogs", type: "error", message: error.message || "Failed delete record" });
        }
    };
