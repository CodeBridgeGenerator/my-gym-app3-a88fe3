...
import MembershiplansDatatable from "./MembershiplansDataTable";
import MembershiplansEditDialogComponent from "./MembershiplansEditDialogComponent";
import MembershiplansCreateDialogComponent from "./MembershiplansCreateDialogComponent";
import MembershiplansFakerDialogComponent from "./MembershiplansFakerDialogComponent";
import MembershiplansSeederDialogComponent from "./MembershiplansSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const MembershiplansPage = (props) => {
    ...
    const filename = "membershiplans.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("membershiplans");
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("membershiplans").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Membershiplans", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("membershiplans").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Membershiplans",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/membershiplans/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="membershiplans-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="membershiplans-datatable">
                    <MembershiplansDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <MembershiplansEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <MembershiplansCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <MembershiplansFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <MembershiplansSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(MembershiplansPage);
