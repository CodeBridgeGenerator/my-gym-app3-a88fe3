...
import membershiplansFakerFactory from "./membershiplansFakerFactory";
...
const MembershiplansFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = membershiplansFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("membershiplans").create(elem));
...
export default connect(mapState, mapDispatch)(MembershiplansFakerDialogComponent);
